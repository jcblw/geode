...
var Geode  = function(username, local){
	...
	that.merge = function(){
		if(typeof arguments[0] === 'object' && !object[0].length){
	...
	for(var i = 0; i < that.methods; i += 1){
		if(!that[that.methods[i]]){
			that[that.methods[i]] = function(data, callback){
				that.request(that.methods[i], data, callback);
			};
		}
	};
